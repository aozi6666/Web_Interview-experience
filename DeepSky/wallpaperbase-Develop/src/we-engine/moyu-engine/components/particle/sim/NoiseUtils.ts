...
export function perlinNoise3D(x: number, y: number, z: number): number {
  ...
  return lerp(
    lerp(
      lerp(dot3(gradients3[perm[AA] % 12], x, y, z), dot3(gradients3[perm[BA] % 12], x - 1, y, z), u),
      lerp(dot3(gradients3[perm[AB] % 12], x, y - 1, z), dot3(gradients3[perm[BB] % 12], x - 1, y - 1, z), u),
      v,
    ),
    lerp(
      lerp(dot3(gradients3[perm[AA + 1] % 12], x, y, z - 1), dot3(gradients3[perm[BA + 1] % 12], x - 1, y, z - 1), u),
      lerp(dot3(gradients3[perm[AB + 1] % 12], x, y - 1, z - 1), dot3(gradients3[perm[BB + 1] % 12], x - 1, y - 1, z - 1), u),
      v,
    ),
    w,
  );
...
export function curlNoise(x: number, y: number, z: number): [number, number, number] {
  ...
  const dPx_dz = (perlinNoise3D(x, y, z + e) - perlinNoise3D(x, y, z - e)) / (2 * e);
  const dPy_dx = (perlinNoise3D(x + e + ox1, y + oy1, z + oz1) - perlinNoise3D(x - e + ox1, y + oy1, z + oz1)) / (2 * e);
  const dPy_dz = (perlinNoise3D(x + ox1, y + oy1, z + e + oz1) - perlinNoise3D(x + ox1, y + oy1, z - e + oz1)) / (2 * e);
  const dPz_dy = (perlinNoise3D(x + ox2, y + e + oy2, z + oz2) - perlinNoise3D(x + ox2, y - e + oy2, z + oz2)) / (2 * e);
  const dPz_dx = (perlinNoise3D(x + e + ox2, y + oy2, z + oz2) - perlinNoise3D(x - e + ox2, y + oy2, z + oz2)) / (2 * e);
  const dPx_dy = (perlinNoise3D(x, y + e, z) - perlinNoise3D(x, y - e, z)) / (2 * e);
