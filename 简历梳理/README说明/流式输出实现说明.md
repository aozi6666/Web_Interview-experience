...
【用户在 UI 里输入】“解释一段 JS”
   ...
   |    （你们通常是 BotManager/Server API 去发起这轮）
   v
【后端 Bot / 对话编排器】
   |
   | ③ 后端准备请求 LLM：
   |    - 拼系统提示词 SystemMessages（你 SetLLM 里那一堆）
   |    - 带上 HistoryLength 的历史上下文
   |    - 带上用户问题 “解释一段 JS”
   |    - 指定要“流式输出”(stream=true / 或类似配置)
   v
【AI 供应商 / LLM 服务】
   |
   | ④ LLM 开始生成回答（不是一次性给完）
...
【后端 Bot / 对话编排器】
   |
   | ⑤ 后端一边接收 LLM 的流式片段，一边做“整理/切片”：
   |    - 给这一轮分配 roundId = N
   |    - 给每个片段递增 sequence = 1,2,3...
   |    - 生成字幕 item:
   |      { text, userId=botUserId, roundId, sequence, definite, paragraph }
   |
   |    说明：
   |    - definite=false 表示还在生成中
   |    - paragraph=true/false 表示段落是否结束（你们用它判断收尾）
   v
【后端把字幕通过 RTC 发进房间】
   |
   | ⑥ 后端把多个字幕 item 打包成：
...
【App.tsx 的 OnSubtitleDetailed：流式显示】
   |
   | ⑨ App.tsx 做 UI：
   |    - roundId 变了 -> 新开一条 AI 气泡（addMessage answer streaming）
   |    - text 来一段 -> 拼进缓存 -> updateMessage（于是“边打字边出现”）
   |    - 收到 definite=true && paragraph=true -> 标 finished + 清理缓存
   v
【用户看到 AI 回复一边生成一边出现】
